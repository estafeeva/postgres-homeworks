...
SELECT customers.company_name, CONCAT(first_name, ' ', last_name) AS first_last_name FROM customers
...
-- которые не сняты с продажи (поле discontinued) и которых меньше 25 и которые в категориях Dairy
Products и Condiments.
...
INNER JOIN (SELECT DISTINCT order_details.product_id FROM order_details
